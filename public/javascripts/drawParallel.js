...
function drawPath(){
    ...
    function mainPathClick(this1, event, pathCount){
        ...
        self.parallelPath = self.group.append('g').attr('class', 'perallelPath')

        let parallelEndPoints = []
        let newParallelPoint = (self.endPointGroup.append('circle').attr('class', 'endPoint'))
        parallelEndPoints.push(newParallelPoint)
        parallelEndPointGroups.push(parallelEndPoints)

        let pathData1 = pathDatas[0][0].coords
        let pathData2 = pathDatas[0][1].coords
        let pathData3 = pathDatas[0][2].coords

        const distance = 55;

        let sin = distance * Math.sin(Math.atan(pathData2.y, pathData2.x))//for x
        let cos = distance * Math.cos(Math.atan(pathData2.y, pathData2.x))//for y

        let parX = pathData2.x + sin
        let parY = pathData2.y - cos

        updateSVG2(parallelEndPointGroups[thisCount], parX, parY)
    }
}
...
function updateSVG2(parallelEndPointsArray, assX, assY) {
    // PARALLEL END POINTS
    // for (let i = 0; i < endPointsArray.length; i++) {
        let endPoint = d3.select(parallelEndPointsArray[0]._groups[0][0])
        endPoint.attr('r', 5)
            .attr('cx', assX)
            .attr('cy', assY)
            .attr('fill', 'green')
