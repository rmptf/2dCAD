...
d3.select("body").insert("div")
    .append("button")
    .text("Add something")
    .on("click", addSomething);

function addSomething(){
    let self = this, singlePointData = [],  m1, isDown = false, isDrag = false;
    svg.on('mousedown', function() {
        m1 = d3.mouse(this);
        if (!isDown && !isDrag) {
            self.singlePointData = [{ x: m1[0], y: m1[1] }];
            self.pointElementSP = svg.append('circle').attr('class', 'pointC').call(dragSP);
            updateSVG()
            isDrag = false;
        } else {
            isDrag = true;
    ...
    let dragSP = d3.behavior.drag().on('drag', dragPointSP);

    function dragPointSP() {
        let e = d3.event;
        d3.select(self.pointElementSP[0][0])
            .attr('cx', self.singlePointData[0].x += e.dx )
            .attr('cy', self.singlePointData[0].y += e.dy );
        updateSVG();   
    }

    function updateSVG() {
        let pointCP = d3.select(self.pointElementSP[0][0]).data(self.singlePointData);
        pointCP.attr('r', 5)
                .attr('cx', self.singlePointData[0].x)
                .attr('cy', self.singlePointData[0].y)
                .attr('fill', 'red');
    }
}
