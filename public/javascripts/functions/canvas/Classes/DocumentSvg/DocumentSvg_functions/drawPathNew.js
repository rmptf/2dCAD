...
function drawPath(event, documentSvgFigures, pathDrawingData, documentSvgD3, actionStates) {
    ...
    documentSvgD3.on("dblclick", () => svg_dblClick(documentSvgD3, actionStates, pathDrawingData))
    if(pathDrawingData.isDown === false) {
        ...
        // documentSvgD3.on("mousemove", (event) => {event, newFigure.svg_mouseMove(event, pathDrawingData.isDown)})
        ...
        let additionalSecondaryPath = thisFigure.createPath_secondary(thisFigure.svgGroups.secondarySvgGroupElements[1], thisFigure)
        let firstTwoEndPoints = thisFigure.createEndPoint_primary(thisFigure.svgGroups.secondarySvgGroupElements[2], thisFigure, additionalPathData, additionalSecondaryPath)
...
// place this function in documentSvg class
function svg_dblClick(documentSvgD3, actionStates, pathDrawingData) {
